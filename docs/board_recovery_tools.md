...
| source-linked owner-closure provenance | `rtk python tools/source_linked_owner_closure.py --context CONTEXT.json [--focus-symbol SYMBOL --objdiff-canonical-sha256 SHA256] [--output RESULT.json] [--require-closure]`; `rtk python tools/crack_learning_rules.py --report REPORT.json --function SYMBOL --source-linked-owner-closure-context CONTEXT.json` | `0fac65da27c93a12407de8c2f5470b13b5fcfe97` | `tools/source_linked_owner_closure.py`, `tools/crack_learning_rules.py`, `tools/tests/test_source_linked_owner_closure.py` | Rejects fallback-linked retail checksums, binds a `Matching` owner through exact candidate path/hash, clean link manifest, and manifest-bound retail outputs, and optionally diagnoses one sealed live addressable read-only f32 owner. It never grants closure or promotion authority. | [Source-linked owner-closure provenance gate](crack_learning_rules.md#source-linked-owner-closure-provenance-gate) |
| immutable same-file history contract closure | `rtk python tools/same_file_history_contract_closure.py --repo REPO --commit COMMIT --path FILE --function SYMBOL --graphify-location LOCATION --report-sha256 SHA256 --destination SOURCE --expect-function-sha256 SHA256 --expect-destination-sha256 SHA256 --require-contract SYMBOL=KIND --output MANIFEST.json`; `rtk python tools/crack_learning_rules.py --report REPORT.json --function SYMBOL --same-file-history-contract-context CONTEXT.json` | `bea74ceb887c7fc0b719346174a76111fbf0d4ec` | `tools/same_file_history_contract_closure.py`, `tools/crack_learning_rules.py`, `tools/tests/test_same_file_history_contract_closure.py` | Reads one preselected immutable same-file donor blob, extracts one function, recursively closes only referenced macros/types/externs/static objects/pool owners, filters exact destination contracts, and emits one manifest-ordered missing package. Ambiguity, incompatibility, omission, broad search, body-only retry, shaping, or authority advancement fails closed. | [Immutable same-file history contract closure](crack_learning_rules.md#immutable-same-file-history-contract-closure) |
| single-use final-call direct consumer | `rtk python tools/crack_learning_rules.py --report REPORT.json --function SYMBOL --single-use-final-call-context CONTEXT.json` | `7cb46ca59cfcce57a09f97f5ef993c47c4d652f5` | `tools/single_use_final_call_consumer.py`, `tools/crack_learning_rules.py`, `tools/tests/test_single_use_final_call_consumer.py` | Maps an otherwise exact four-row two-saved-GPR swap to one long-lived parameter and one assigned-once/consumed-once scalar conversion at the final typed semantic call, then ranks only direct producer consumption. Restore thunks are not semantic calls; later ordinary calls, proof drift, shaping, and authority advancement fail closed. | [Single-use final-call direct-consumer diagnosis](crack_learning_rules.md#single-use-final-call-direct-consumer-diagnosis) |
| source-bound MWCC score-delta control | `rtk python tools/crack_learning_rules.py --report REPORT.json --function SYMBOL --mwcc-score-delta-context CONTEXT.json` | `75812cddcbe00cdebcca47ec706ba527d159a886` | `tools/mwcc_score_delta_reducer.py`, `tools/crack_learning_rules.py`, `tools/tests/test_mwcc_score_delta_reducer.py` | Joins a closed exact-size/frame/relocation allocation residual to same-session MWCC scores and a changed physical color, then requires every minimal +def/+use increment to be sealed to one exact source span. The output is a diagnostic control only: source debt, retention, promotion, repeat tracing, and unrelated permutations remain suppressed. | [Source-bound MWCC score-delta control](crack_learning_rules.md#source-bound-mwcc-score-delta-control) |
| switch/default topology then typed fold | `rtk python tools/crack_learning_rules.py --report REPORT.json --function SYMBOL --switch-default-fold-context CONTEXT.json` | `89dae64fafe7527751b5e8db1a6c019633962bee` | `tools/switch_default_constant_fold.py`, `tools/crack_learning_rules.py`, `tools/tests/test_switch_default_constant_fold.py` | Requires a four-byte terminal-default branch seam with exact frame/homes/relocations, closes that topology first, then admits one natural f32-vs-f64 source fold only when the resulting typed pool residual is exactly one unique-consumer row. Guard permutations, opaque bits, numeric search, pool reshaping, tracing, and authority advancement remain suppressed. | [Switch/default topology and typed constant-fold diagnosis](crack_learning_rules.md#switchdefault-topology-and-typed-constant-fold-diagnosis) |
