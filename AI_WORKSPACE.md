...
- no agent prompts, queue metadata, knowledge-card machinery, benchmark harness,
  AI attribution, or orchestration documentation.
...
Its history and files are intentionally AI-forward. They stay on this branch.
...
historical marker of this boundary, not a merge candidate.
...
The transfer must start from a fresh branch and worktree based on current
`main`. It must copy only explicitly selected C files from the verified worker
commit and prove that each promoted Git blob is identical to that commit.
...
The promotion branch is based on `main`, so none of the AI workspace files are
present. The tool rejects:
...
configuration update. Those changes are **not** imported automatically from the
AI branch.

They must be recreated from a clean `main` checkout and reviewed as ordinary
human-facing project changes. This prevents prompts, scaffolding, generated
metadata, speculative names, and agent-specific structure from leaking into the
...
workspace. They are not committed to the clean promotion branch.
