...
typedef struct {
    ...
    /* 0xAC */ u32 dvdBusyTime;
} MSM_STREAM_SLOT; // Size 0xB0
...
s32 msmStreamGetStatus(int streamNo) {
    ...
    if (slot->updateAramF == TRUE) {
        status = MSM_STREAM_DVDERROR;
    } else if (slot->pauseF == TRUE) {
...
void msmStreamPeriodicProc(void) {
    ...
    for (i = 0; i < StreamInfo.header.chanMax; i++) {
        ...
        if (slot->updateAramF == TRUE) {
            slot->pauseTime = 1;
            if (DVDGetCommandBlockStatus(&slot->file.cb) == 1 && ++slot->dvdBusyTime > 180) {
        ...
        if (slot->status != 0 || slot->pauseF == TRUE) {
            StreamInfo.time++;
        }
    }
}
...
static void msmStreamPauseOff(s32 streamNo) {
    ...
    if (slot->pauseTimeMax != 0) {
        ...
        slot->readBusyF = TRUE;
        DVDReadAsyncPrio(&slot->file, slot->streamBuf, readSize,
                         slot->streamBaseOfs + (slot->streamPos - readSize),
                         msmStreamDvdCallback, 1);
    }
}
...
static s32 msmStreamPackStartStereo(s32 streamId, MSM_STREAMPARAM *param, s32 sampleOfs) {
    ...
    slotL->readBusyF = 1;
    DVDReadAsyncPrio(&slotL->file, slotL->streamBuf, sizeL,
                     slotL->streamBaseOfs + (slotL->streamPos - sizeL),
                     msmStreamDvdCallback, 1);
    slotL->dvdBusyTime = 0;
    slotR->readBusyF = 1;
    DVDReadAsyncPrio(&slotR->file, slotR->streamBuf, sizeR,
                     slotR->streamBaseOfs + (slotR->streamPos - sizeR),
                     msmStreamDvdCallback, 1);
    slotR->dvdBusyTime = 0;
    return chanL;
}
...
static s32 msmStreamPackStartMono(s32 streamId, MSM_STREAMPARAM *param, s32 sampleOfs) {
    ...
    slot->streamPos += size;
    slot->readBusyF = TRUE;
    DVDReadAsyncPrio(&slot->file, slot->streamBuf, size,
                     slot->streamBaseOfs + (slot->streamPos - size),
                     msmStreamDvdCallback, 1);
    slot->dvdBusyTime = 0;
    return chan;
}
                ...
                if (slot->streamPos >= slot->loopLen) {
                    if (slot->stereoF != 0) {
                        ...
                        if (slot->streamReadSize != 0) {
                            slot->readBusyF = 1;
                            DVDReadAsyncPrio(fileInfo, slot->streamReadBuf, slot->streamReadSize,
                                             slot->streamBaseOfs + slot->streamPos,
                                             msmStreamDvdCallback2, 1);
