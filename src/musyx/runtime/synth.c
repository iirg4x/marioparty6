...
#define SYNTH_FX_SOURCE_ID_FLAG ((u32)2147483648U)

#define SYNTH_MIDI_CHANNEL_NONE 255
#define SYNTH_MIDI_SET_NONE 255
#define SYNTH_JOB_INDEX_NONE 255
#define SYNTH_TRACK_NONE 255
#define SYNTH_PARAMETER_NONE 255
#define SYNTH_PRIORITY_MAX 255
#define SYNTH_VOICE_INDEX_MASK 255
#define SYNTH_LOW_BYTE_MASK 255
#define SYNTH_KEY_VALUE_MASK 127
#define SYNTH_VOLUME_MAX 127
#define SYNTH_PANNING_MAX 127
#define SYNTH_KEY_FLAG 128
#define SYNTH_PANNING_FIXED_FLAG 128
#define SYNTH_PAN_CENTER 64
#define SYNTH_LAYER_ID_NONE 65535
#define SYNTH_U16_MASK 65535

#define SYNTH_LAYER_TYPE_MASK 49152
#define SYNTH_KEYMAP_TYPE 16384
#define SYNTH_LAYER_TYPE 32768
#define SYNTH_MIDI_CTRL_CENTER 8192
#define SYNTH_MODULATION_MASK_MODERN 131071
#define SYNTH_MODULATION_MASK_LEGACY 511
#define SYNTH_CONTROL_THRESHOLD 8064
#define SYNTH_PAN_INPUT_SCALE 512
#define SYNTH_PAN_OUTPUT_MAX 8323072
#define SYNTH_PAN_OUTPUT_MAX_UNSIGNED 8323072u
#define SYNTH_PAN_OUTPUT_CENTER 4194304
#define SYNTH_SWEEP_TIME_MASK 16777215
#define SYNTH_SWEEP_FACTOR_MASK 268435455
#define SYNTH_JOB_SLOT_MASK 31
#define SYNTH_JOB_DELAY 3840
#define SYNTH_DIRTY_FLAGS_FULL_MODERN 32767
#define SYNTH_DIRTY_FLAGS_FULL_LEGACY 8191
#define SYNTH_LFO_VALUE_MAX 32767
#define SYNTH_FILTER_SWITCH_MAX 8191
#define SYNTH_DIRTY_FLAGS_PAN_VOLUME 6
#define SYNTH_DIRTY_FLAGS_FILTER 24576
#define SYNTH_DIRTY_FLAGS_OUTPUT 3841

#define SYNTH_CFLAG_PORTAMENTO_ACTIVE 1024
#define SYNTH_CFLAG_PORTAMENTO_INIT 4096
#define SYNTH_CFLAG_PORTAMENTO_LEGATO 16
#define SYNTH_CFLAG_PORTAMENTO_ROOT 131072
#define SYNTH_CFLAG_PORTAMENTO_REJECT 131074
#define SYNTH_CFLAG_VIBRATO 8192
#define SYNTH_CFLAG_VIBRATO_OFFSET 16384
#define SYNTH_CFLAG_PORTAMENTO_STATE_MASK 135168
#define SYNTH_CFLAG_ENVELOPE 32768
#define SYNTH_CFLAG_PAN_DIRTY 35184372088832LL
#define SYNTH_CFLAG_PITCH_ADSR 2199023255552LL
#define SYNTH_CFLAG_VOLUME_DIRTY 17592186044416LL
#define SYNTH_CFLAG_EVENT_GUARD_MASK 1099511627784LL
#define SYNTH_CFLAG_EVENT_GUARD_VALUE 8
#define SYNTH_CFLAG_KEY_PENDING 32
#define SYNTH_CFLAG_KEY_ACTIVE 16
#define SYNTH_CFLAG_EVENT_MASK 1099511627920LL
#define SYNTH_CFLAG_EVENT_KEYOFF 144
#define SYNTH_CFLAG_MIDI_SETUP 2
#define SYNTH_CFLAG_PITCH_BEND_MODERN 65584
#define SYNTH_CFLAG_PITCH_BEND_LEGACY 65552
...
void synthInitPortamento(SYNTH_VOICE* svoice) {
  if (svoice->cFlags & SYNTH_CFLAG_PORTAMENTO_ROOT) {
    return;
  }

  if (svoice->portType == 1) {
    if (!(svoice->cFlags & SYNTH_CFLAG_PORTAMENTO_INIT)) {
      svoice->portTime = 0;
    } else {
      svoice->portTime = svoice->portDuration;
    }
...
static u32 do_voice_portamento(u8 key, u8 midi, u8 midiSet, u32 isMaster, u32* rejected) {
  ...
  for (i = 0, sv = synthVoice; i < synthInfo.voiceNum; ++i, ++sv) {
    if (
      ...
      if ((sv->cFlags & SYNTH_CFLAG_PORTAMENTO_LEGATO) != 0 && (sv->cFlags & SYNTH_CFLAG_EVENT_GUARD_MASK) != SYNTH_CFLAG_EVENT_GUARD_VALUE && hwIsActive(i)) {
        if (vid == SND_ID_ERROR && (sv->cFlags & SYNTH_CFLAG_PORTAMENTO_REJECT) == SYNTH_CFLAG_PORTAMENTO_REJECT) {
          *rejected = TRUE;
          return SND_ID_ERROR;
        }
        last_sv = sv;
        ...
        sv->curNote = key + ((sv->curNote & SYNTH_LOW_BYTE_MASK) - sv->orgNote);
        sv->orgNote = key;
        sv->curDetune = 0;
        sv->portTime = 0;
        sv->cFlags |= SYNTH_CFLAG_PORTAMENTO_ROOT;
        ...
        if (vid == SND_ID_ERROR) {
          ...
          synthVoice[id & SYNTH_VOICE_INDEX_MASK].child = sv->id;
          sv->parent = id;
          id = sv->id;
...
static u32 check_portamento(u8 key, u8 midi, u8 midiSet, u32 newVID, u32* vid) {
  ...
  if (inpGetMidiCtrl(65 /* TODO SND_MIDICTRL_? */, midi, midiSet) > 8064) {
    *vid = do_voice_portamento(key & SYNTH_KEY_VALUE_MASK, midi, midiSet, newVID, &rejected);
    return !rejected;
  }
  *vid = SND_ID_ERROR;
  return 1;
...
static u32 StartLayer(u16 layerID, s16 prio, u8 maxVoices,
  ...
  mKey = key & SYNTH_KEY_VALUE_MASK;
  for (; n != 0; --n, ++l) {
    if (l->id == SYNTH_LAYER_ID_NONE || l->keyLow > mKey || l->keyHigh < mKey) {
      continue;
    }
    ...
    if ((l->id & SYNTH_LAYER_TYPE_MASK) == 0) {
  ...
  start_new_id:
    if ((l->panning & SYNTH_PANNING_FIXED_FLAG) == 0) {
      p = l->panning - SYNTH_PAN_CENTER;
      p += panning;
      // TODO
      // p = CLAMP(p, 0, 0x7f);
      p = CLAMP_INV(p, 0, SYNTH_PANNING_MAX);
    } else {
      p = SYNTH_PANNING_FIXED_FLAG;
    }

    v = (vol * l->volume) / SYNTH_VOLUME_MAX;
    prio += l->prioOffset;
    prio = CLAMP(prio, 0, SYNTH_PRIORITY_MAX);

    switch (l->id & SYNTH_LAYER_TYPE_MASK) {
    case 0:
      new_id = macStart(l->id, prio, maxVoices, allocId, k | key & SYNTH_KEY_FLAG, v, p, midi, midiSet,
                        section, step, trackid, 0, vGroup, studio, itd);
      break;
    case SYNTH_KEYMAP_TYPE:
      new_id = StartKeymap(l->id, prio, maxVoices, allocId, k | key & SYNTH_KEY_FLAG, v, p, midi, midiSet,
                           section, step, trackid, 0, vGroup, studio, itd);
      break;
    case SYNTH_LAYER_TYPE:
      new_id = StartLayer(l->id, prio, maxVoices, allocId, k | key & SYNTH_KEY_FLAG, v, p, midi, midiSet,
                          section, step, trackid, 0, vGroup, studio, itd);
      break;
    }
    ...
    apply_new_id:
      if (vid == SND_ID_ERROR) {
        if (vidFlag != 0) {
          vid = vidMakeRoot(&synthVoice[new_id & SYNTH_VOICE_INDEX_MASK]);
        } else {
          vid = new_id;
        }
      } else {
        synthVoice[id & SYNTH_VOICE_INDEX_MASK].child = new_id;
        synthVoice[new_id & SYNTH_VOICE_INDEX_MASK].parent = id;
      }
      id = new_id;
      while (synthVoice[id & SYNTH_VOICE_INDEX_MASK].child != SND_ID_ERROR) {
#if MUSY_VERSION >= MUSY_VERSION_CHECK(1, 5, 4)
        synthVoice[id & SYNTH_VOICE_INDEX_MASK].block = 1;
#endif
        id = synthVoice[id & SYNTH_VOICE_INDEX_MASK].child;
      }
#if MUSY_VERSION >= MUSY_VERSION_CHECK(1, 5, 4)
      synthVoice[id & SYNTH_VOICE_INDEX_MASK].block = 1;
#endif
...
static u32 StartKeymap(u16 keymapID, s16 prio, u8 maxVoices,
  ...
  if ((keymap = dataGetKeymap(keymapID)) != NULL) {
    o = key & SYNTH_KEY_VALUE_MASK;
    if (keymap[o].id != SYNTH_LAYER_ID_NONE && (keymap[o].id & SYNTH_LAYER_TYPE_MASK) != SYNTH_KEYMAP_TYPE) {
      if ((keymap[o].panning & SYNTH_PANNING_FIXED_FLAG) == 0) {
        p = (keymap[key].panning - SYNTH_PAN_CENTER);
        p += panning;
        if (p < 0) {
          panning = 0;
        } else if (p > SYNTH_PANNING_MAX) {
          panning = SYNTH_PANNING_MAX;
        } else {
          panning = p;
        }
      } else {
        panning = SYNTH_PANNING_FIXED_FLAG;
      }

      k = (key & SYNTH_KEY_VALUE_MASK) + keymap[o].transpose;
      k = CLAMP(k, 0, 127);

      prio += keymap[o].prioOffset;
      prio = CLAMP(prio, 0, SYNTH_PRIORITY_MAX);

      if ((keymap[o].id & SYNTH_LAYER_TYPE_MASK) == 0) {
        if (!check_portamento(k & SYNTH_LOW_BYTE_MASK, midi, midiSet, vidFlag, &vid)) {
          return SND_ID_ERROR;
        }
        if (vid != SND_ID_ERROR) {
          return vid;
        }
        return macStart(keymap[o].id, prio, maxVoices, allocId, k | key & SYNTH_KEY_FLAG, vol, panning, midi,
                        midiSet, section, step, trackid, vidFlag, vGroup, studio, itd);
      }

      return StartLayer(keymap[o].id, prio, maxVoices, allocId, k | key & SYNTH_KEY_FLAG, vol, panning, midi,
                        midiSet, section, step, trackid, vidFlag & SYNTH_LOW_BYTE_MASK, vGroup, studio, itd);
    }
  }
...
static void unblockAllAllocatedVoices(u32 vid) {
  ...
  while (id != SND_ID_ERROR) {
    synthVoice[id & SYNTH_VOICE_INDEX_MASK].block = 0;
    id = synthVoice[id & SYNTH_VOICE_INDEX_MASK].child;
  }
}
#endif
...
u32 synthStartSound(u16 id, u8 prio, u8 max,
  ...
  switch (id & SYNTH_LAYER_TYPE_MASK) {
  case 0: {
    u32 vid; // r1+0x34
...
static u32 convert_cents(SYNTH_VOICE* svoice, u32 ccents) {
  ...
  if ((curDetune = ccents & SYNTH_U16_MASK) != 0) {
    cpitch += curDetune * ((sndPitchUpOne(cpitch / 65536) & SYNTH_U16_MASK) - (cpitch / 65536 & SYNTH_U16_MASK));
  }
  return cpitch;
}
...
static void UpdateTimeMIDICtrl(SYNTH_VOICE* sv) {
  ...
  sv->midiDirtyFlags = SYNTH_DIRTY_FLAGS_FULL_MODERN;
#else
  sv->midiDirtyFlags = SYNTH_DIRTY_FLAGS_FULL_LEGACY;
#endif
}
...
static void LowPrecisionHandler(u32 i) {
  ...
  if ((sv->cFlags & SYNTH_CFLAG_VIBRATO) != 0) {
...
#else
    cntDelta = (lowDeltaTime & SYNTH_SWEEP_TIME_MASK) << 4;
    addFactor = lowDeltaTime & SYNTH_SWEEP_FACTOR_MASK;
#endif
    for (j = 0; j < 2; ++j) {
      if (sv->sweepNum[j] == 0) {
        continue;
  ...
  for (j = 0; j < 2; ++j) {
    ...
    if ((s32)sv->panTime[j] <= 0) {
      ...
      sv->panning[j] = CLAMP_INV((s32)sv->panning[j], 0, SYNTH_PAN_OUTPUT_MAX_UNSIGNED);
    }

    sv->cFlags |= SYNTH_CFLAG_PAN_DIRTY;
  }

  if ((sv->cFlags & SYNTH_CFLAG_PITCH_ADSR) != 0 &&
      adsrHandleLowPrecision(&sv->pitchADSR, &adsr_start, &adsr_delta)) {
    sv->cFlags &= ~SYNTH_CFLAG_PITCH_ADSR;
  }

  ccents = sv->curNote * 65536 + (sv->curDetune * 65536) / 100;
  if ((sv->cFlags &
#if MUSY_VERSION >= MUSY_VERSION_CHECK(1, 5, 4)
       SYNTH_CFLAG_PITCH_BEND_MODERN
#else
       SYNTH_CFLAG_PITCH_BEND_LEGACY
#endif
       ) != 0) {
    if (sv->midi != SYNTH_MIDI_CHANNEL_NONE) {
    ...
    pbend = sv->pbLast;
  pbend_adjust:
    if (pbend != SYNTH_MIDI_CTRL_CENTER) {
      pbend -= SYNTH_MIDI_CTRL_CENTER;
      if (pbend < 0) {
  ...
  if ((sv->cFlags & SYNTH_CFLAG_VIBRATO) != 0) {
    ...
    if (sv->vibModAddScale != 0) {
      ...
      vrange += (sv->vibModAddScale * ((Modulation & SYNTH_MODULATION_MASK_MODERN) >> 7)) >> 7;
#else
      vrange += (sv->vibModAddScale * ((Modulation >> 7) & SYNTH_MODULATION_MASK_LEGACY)) >> 7;
#endif
    }
    if ((sv->cFlags & SYNTH_CFLAG_VIBRATO_OFFSET) != 0) {
#if MUSY_VERSION >= MUSY_VERSION_CHECK(2, 0, 3)
      voff = (sv->vibCurOffset * (Modulation >> 7)) >> 7;
#elif MUSY_VERSION >= MUSY_VERSION_CHECK(1, 5, 4)
      voff = (sv->vibCurOffset * ((Modulation & SYNTH_MODULATION_MASK_MODERN) >> 7)) >> 7;
#else
      voff = (sv->vibCurOffset * ((Modulation >> 7) & SYNTH_MODULATION_MASK_LEGACY)) >> 7;
#endif
    } else {
  ...
  if (sv->midi != SYNTH_MIDI_CHANNEL_NONE) {
    ...
    if (portamento != sv->portLastCtrlState || (sv->cFlags & SYNTH_CFLAG_PORTAMENTO_STATE_MASK) == SYNTH_CFLAG_PORTAMENTO_ROOT) {
      if (portamento <= SYNTH_CONTROL_THRESHOLD) {
        sv->cFlags &= ~SYNTH_CFLAG_PORTAMENTO_ACTIVE;
      } else {
        if ((sv->cFlags & SYNTH_CFLAG_PORTAMENTO_ACTIVE) == 0) {
          synthInitPortamento(sv);
        }
        sv->cFlags |= SYNTH_CFLAG_PORTAMENTO_ACTIVE;
      }
      sv->cFlags |= SYNTH_CFLAG_PORTAMENTO_INIT;
  ...
  hwSetPitch(i, cpitch);
  synthAddJob(sv, 0, SYNTH_JOB_DELAY);
...
static void ZeroOffsetHandler(u32 i) {
  ...
  if ((synthFlags & 1) == 0) {
    if ((sv->cFlags & SYNTH_CFLAG_PAN_DIRTY) != 0 || (sv->midiDirtyFlags & SYNTH_DIRTY_FLAGS_PAN_VOLUME) != 0) {
      sv->cFlags &= ~SYNTH_CFLAG_PAN_DIRTY;
      pan = sv->panning[0] + (inpGetPanning(sv) - 8192) * SYNTH_PAN_INPUT_SCALE;
      sv->lastPan = CLAMP_INV(pan, 0, SYNTH_PAN_OUTPUT_MAX);

      if ((synthFlags & 2) != 0) {
        if ((sv->lastSPan = sv->panning[1] + inpGetSurPanning(sv) * SYNTH_PAN_INPUT_SCALE) > SYNTH_PAN_OUTPUT_MAX) {
          sv->lastSPan = SYNTH_PAN_OUTPUT_MAX;
        }
      } else {
        sv->lastSPan = 0;
      }
    ...
    sv->lastPan = SYNTH_PAN_OUTPUT_CENTER;
    sv->lastSPan = 0;
    volUpdate |= (sv->cFlags & SYNTH_CFLAG_PAN_DIRTY) != 0;
    sv->cFlags &= ~SYNTH_CFLAG_PAN_DIRTY;
  }

  if (volUpdate || (sv->midiDirtyFlags & SYNTH_DIRTY_FLAGS_OUTPUT) != 0) {
    preVol = voiceVol;
...
#if MUSY_VERSION >= MUSY_VERSION_CHECK(2, 0, 3)
  if (sv->midiDirtyFlags & SYNTH_DIRTY_FLAGS_FILTER) {
    if (inpGetFilterSwitch(sv) > SYNTH_FILTER_SWITCH_MAX) {
...
static void EventHandler(u32 i) {
  ...
  macSetPedalState(sv, inpGetPedal(sv) > SYNTH_CONTROL_THRESHOLD);

  if ((sv->cFlags & SYNTH_CFLAG_KEY_PENDING) != 0) {
    sv->cFlags &= ~SYNTH_CFLAG_KEY_PENDING;
    sv->cFlags |= SYNTH_CFLAG_KEY_ACTIVE;
    hwStart(i, sv->studio);
  }

  if ((sv->cFlags & SYNTH_CFLAG_EVENT_MASK) == SYNTH_CFLAG_EVENT_KEYOFF) {
    sv->cFlags &= ~SYNTH_CFLAG_EVENT_KEYOFF;
    hwKeyOff(i);
    if ((sv->cFlags & SYNTH_CFLAG_PITCH_ADSR) != 0 && adsrRelease(&sv->pitchADSR)) {
      sv->cFlags &= ~SYNTH_CFLAG_PITCH_ADSR;
    }
  }
...
static void synthAddJob(SYNTH_VOICE* svoice, SYNTH_JOBTYPE jobType, u32 deltaTime) {
  ...
  jobTabIndex = ((deltaTime / 256) + synthJobTableIndex) & SYNTH_JOB_SLOT_MASK;
  jobTab = &synthJobTable[jobTabIndex];

  switch (jobType) {
  case SYNTH_JOBTYPE_LOW:
    newJq = &svoice->lowPrecisionJob;
    if (newJq->jobTabIndex != SYNTH_JOB_INDEX_NONE) {
...
static void HandleJobQueue(SYNTH_QUEUE** queueRoot, void (*handler)(u32)) {
  ...
  while (jq != NULL) {
    nextJq = jq->next;
    jq->jobTabIndex = SYNTH_JOB_INDEX_NONE;
...
void synthHandle(u32 deltaTime) {
  ...
  if (hwGetTimeOffset() == 0) {
    ...
    for (s = 0; s < 8; ++s) {
      if (synthAuxAMIDI[s] != SYNTH_MIDI_CHANNEL_NONE) {
      ...
      if (synthAuxBMIDI[s] != SYNTH_MIDI_CHANNEL_NONE) {
...
u32 synthFXStart(u16 fid, u8 vol, u8 pan, u8 studio, u32 itd) {
  ...
  v = SND_ID_ERROR;
  if ((fx = dataGetFX(fid)) != NULL) {
    if (vol == SYNTH_PARAMETER_NONE) {
      vol = fx->volume;
    }

    if (pan == SYNTH_PARAMETER_NONE) {
      pan = fx->panning;
    }

    v = synthStartSound(fx->macro, fx->priority, fx->maxVoices,
#if MUSY_VERSION >= MUSY_VERSION_CHECK(2, 0, 1)
                        fid | SYNTH_FX_SOURCE_ID_FLAG,
#endif
                        fx->key | SYNTH_KEY_FLAG, vol, pan, SYNTH_MIDI_CHANNEL_NONE, SYNTH_MIDI_SET_NONE, 0, 0, SYNTH_TRACK_NONE, fx->vGroup, 0, studio,
                        itd);
  }
...
int synthCheckFXRealloc(unsigned short fid) {
  ...
  allocId = fid | SYNTH_FX_SOURCE_ID_FLAG;
...
bool synthFXSetCtrl(u32 vid, u8 ctrl, u8 value) {
  ...
  while (vid != SND_ID_ERROR) {
    i = vid & SYNTH_VOICE_INDEX_MASK;
    if (vid == synthVoice[i].id) {
      if ((synthVoice[i].cFlags & SYNTH_CFLAG_MIDI_SETUP) != 0) {
...
bool synthSendKeyOff(u32 voiceid) {
  ...
  if (sndActive != 0) {
    ...
    while (voiceid != SND_ID_ERROR) {
      i = voiceid & SYNTH_VOICE_INDEX_MASK;
...
u16 synthGetVolume(u32 vid) {
  ...
  if (vid != SND_ID_ERROR) {
    i = vid & SYNTH_VOICE_INDEX_MASK;
    if (vid == synthVoice[i].id && (synthVoice[i].cFlags & SYNTH_CFLAG_MIDI_SETUP) == 0) {
...
static u32 synthHWMessageHandler(u32 mesg, u32 voiceID) {
  ...
  switch (mesg) {
  case 0:
    if (synthVoice[voiceID & SYNTH_VOICE_INDEX_MASK].block != 0) {
      break;
    }
    vsSampleEndNotify(hwGetVirtualSampleID(voiceID & SYNTH_VOICE_INDEX_MASK));
    if (voiceID != synthVoice[voiceID & SYNTH_VOICE_INDEX_MASK].id) {
      break;
    }
    macSampleEndNotify(&synthVoice[voiceID & SYNTH_VOICE_INDEX_MASK]);
    break;

  case 1:
    voiceKill(voiceID & SYNTH_VOICE_INDEX_MASK);
    break;
...
void synthInit(u32 mixFrq, u32 numVoices) {
  ...
  for (i = 0; i < numVoices; ++i) {
    synthVoice[i].id = SND_ID_ERROR;
    synthVoice[i].cFlags = 0;
    synthVoice[i].age = 0;
    synthVoice[i].prio = 0;
    synthVoice[i].midi = SYNTH_MIDI_CHANNEL_NONE;
    synthVoice[i].volume = 0;
    synthVoice[i].volTable = 0;
    synthVoice[i].revVolScale = 128;
    synthVoice[i].revVolOffset = 0;
    synthVoice[i].panning[0] = synthVoice[i].panTarget[0] = SYNTH_PAN_OUTPUT_CENTER;
    ...
    synthVoice[i].lfo[0].lastValue = SYNTH_LFO_VALUE_MAX;
    synthVoice[i].lfo[1].period = 0;
    synthVoice[i].lfo[1].value = 0;
    synthVoice[i].lfo[1].lastValue = SYNTH_LFO_VALUE_MAX;
    synthVoice[i].portTime = 25600;
    synthVoice[i].portType = 0;
    synthVoice[i].studio = 0;
    synthVoice[i].lowPrecisionJob.voice = i;
    synthVoice[i].lowPrecisionJob.jobTabIndex = SYNTH_JOB_INDEX_NONE;
    synthVoice[i].zeroOffsetJob.voice = i;
    synthVoice[i].zeroOffsetJob.jobTabIndex = SYNTH_JOB_INDEX_NONE;
    synthVoice[i].eventJob.voice = i;
    synthVoice[i].eventJob.jobTabIndex = SYNTH_JOB_INDEX_NONE;
  }
  ...
  for (i = 0; i < 8; ++i) {
    ...
    synthAuxAMIDI[i] = SYNTH_MIDI_CHANNEL_NONE;
    synthAuxBCallback[i] = NULL;
    synthAuxBMIDI[i] = SYNTH_MIDI_CHANNEL_NONE;
