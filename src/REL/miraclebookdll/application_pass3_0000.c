...
void fn_1_A0(void)
{
    ...
    lbl_1_bss_4 = omInitObjMan(0xC8, 0x2000);
    ...
    HuPrcChildCreate(fn_1_1CC, 0x100, 0x3000, 0, HuPrcCurrentGet());
    lbl_1_bss_0 = -1;
...
void fn_1_2AC(OMOBJMAN *objMan)
{
    ...
    fn_1_69A8();
    fn_1_7560();
    HuPrcChildCreate(fn_1_60A4, 100, 0x2000, 0, HuPrcCurrentGet());
...
s32 fn_1_394(void)
{
    ...
    if (lbl_1_bss_AC == -1) {
        lbl_1_bss_AC = HuAudBGMPlay(0x5E);
    }
    fn_1_A30();
    fn_1_4E94();
    if (fn_1_7354() == 0) {
...
void fn_1_4CC(void)
{
    ...
    lbl_1_bss_21C = Hu3DModelCreate(HuDataSelHeapReadNum(
        DATANUM(DATA_miraclebook, 0), 0x10000000, 2));
    ...
    lbl_1_bss_21A = Hu3DModelCreate(HuDataSelHeapReadNum(
        DATANUM(DATA_miraclebook, 1), 0x10000000, 2));
    ...
    lbl_1_bss_218 = Hu3DModelCreate(HuDataSelHeapReadNum(
        DATANUM(DATA_miraclebook, 2), 0x10000000, 2));
    ...
    for (i = 0; i < 2; i++) {
        lbl_1_bss_1C0[i] = Hu3DModelCreate(HuDataSelHeapReadNum(
            DATA_miraclebook + i + 6, 0x10000000, 2));
