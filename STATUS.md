...
`0x20F4` bytes. Wave 97 restores the read-only path-vector contract in
...
Wave 90 restores exact nested helper boundaries and inline objects across the
selection, animation, motion, and configuration flows. `fn_1_4A84`,
`fn_1_B748`, `fn_1_D058`, `fn_1_DA54`, `fn_1_1158C`, `fn_1_12364`,
`fn_1_14510`, `fn_1_19860`, and `fn_1_26328` now pass the relocation-aware
object gate, adding `0x574C` exact target bytes. The owner reaches 227 exact
functions and `0x28C6C` exact bytes; 31 functions and `0x167B8` bytes remain
real compiler-shape work. Eight additional functions retain authenticated
source-shape improvements without receiving Matching credit. The owner remains
fallback-linked `C-not-yet-matched`; no inline assembly, fake local, register
force, or configure flip was admitted. The serialized Ninja and explicit DTK
checksum gates each report 137 files OK; `main.dol` and `mdpartydll.rel` both
compare byte-identical, and the final build leaves no generated `symbols.txt`
hunk. Detailed evidence is in
[`docs/native_matching_wave90.md`](docs/native_matching_wave90.md).

Wave 91 closes four more `REL/mdpartydll/mdparty.c` functions totaling
`0x34F4` target bytes and raises the owner to 231 exact functions and
`0x2C160` exact bytes. Exact same-TU helper boundaries recover the selection,
animation, motion, and configuration flow without fake locals or register
forcing. Six other functions retain target-proven source improvements but no
Matching credit. Detailed evidence is in
[`docs/native_matching_wave91.md`](docs/native_matching_wave91.md).

Wave 92 recovers `fn_1_BBD8`'s result/direction/vector initializer chronology
and folds its `fn_1_BB30` assignment into the target comparison. A bounded
callee-only `dont_inline` diagnostic proves the resulting `fn_1_BBD8` body
byte-exact at `0x580`, but regresses exact `fn_1_BB30` to `0xB4 / 92.238100%`;
the pragma is therefore rejected. Reversed function order and local-`const`
probes also fail object evidence and are absent. Exact counts remain 231 of
258 functions and `0x2C160 / 0x3F424` bytes. The owner remains fallback-linked
...
Wave 97 recovers the read-only `HuVecF` input contract on `fn_1_22A8`, making
the `0x454` function exact while preserving all three exact late-inline
callers. Exact same-translation-unit `fn_1_CA68` callers authenticate the
explicit signed-16-bit character argument at that helper boundary; applying
the same source shape closes `fn_1_156B4` and `fn_1_160A0`, each `0x9EC`
bytes. The owner reaches 251 exact functions and `0x3BE2C` exact target text
bytes, leaving seven functions and `0x35F8` bytes. Neutral zero-comparison,
float-literal, and folded-assignment probes are absent. The source object also
proves that initialized global/table recovery remains real work: `.data` is
`0x1D9` against the target's `0xF10`. No Matching flip, fake local, register
force, inline assembly, synthetic literal global, or pragma is admitted. The
final serialized Ninja build and explicit DTK checksum each report 137 files
...
Wave 99 recovers the pointer-null ARAM contract in `fn_1_2B0` and the copy
expanded into `fn_1_400`. The former integer comparison emitted an immediate
`cmplwi`; the target-proven `(void *)`/`NULL` expression emits the exact
`li`/`cmplw` pair in both bodies. The two functions add `0x39C` exact target
bytes, bringing `mdparty.c` to 253/258 functions and
`0x3C1C8 / 0x3F424` exact text. Five functions and `0x325C` bytes remain
explicit compiler-lifetime residues. Parallel audits leave them unchanged
because the target does not authenticate the ghost vregs, inline threshold,
or synthetic temporaries that would be required to force their remaining
register colors. No inline assembly, fake zero local, volatile/register force,
compiler-control pragma, or Matching flip is admitted. The final serialized
Ninja build and explicit DTK checksum each report 137 files OK; `main.dol` and
